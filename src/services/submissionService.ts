...
import { hasSpecialCharacters, hasScriptTags } from '../../utils/stringUtils.js'
...
function validateSubmissionOrderBy(filters: gqlTypes.SubmissionSearchFilters): Result<unknown> {
    const validateOrderByResults: Result<unknown> = {
        data: undefined,
        hasErrors: false,
        errors: []
    }
    ...
    if (filters.orderBy?.some(order => order.fieldToOrder === 'id')) {
        validateOrderByResults.hasErrors = true
        validateOrderByResults.errors?.push({
            field: 'orderBy',
...
function validateSubmissionSearchBySpokenLanguage(filters: gqlTypes.SubmissionSearchFilters): Result<unknown> {
    const validateSpokenLanguageResults: Result<unknown> = {
        data: undefined,
        hasErrors: false,
        errors: []
    }
    ...
    if (!filters.spokenLanguages) {
        return validateSpokenLanguageResults
    }

    if (filters.spokenLanguages.length == 0) {
        validateSpokenLanguageResults.hasErrors = true
        validateSpokenLanguageResults.errors?.push({
...
function validateSubmissionSearchFilters(filters: gqlTypes.SubmissionSearchFilters): Result<unknown> {
    ...
    const orderByValidationResult = validateSubmissionOrderBy(filters)
    if(orderByValidationResult.hasErrors) {
        validateSearchResults.hasErrors = true
        validateSearchResults.errors = (validateSearchResults.errors || []).concat(orderByValidationResult.errors || [])
    }

    const spokenLanguageResult = validateSubmissionSearchBySpokenLanguage(filters)
    if (spokenLanguageResult.hasErrors) {
        validateSearchResults.hasErrors = true
        validateSearchResults.errors = (validateSearchResults.errors || []).concat(spokenLanguageResult.errors || [])
    }
    ...
    if (filters.googleMapsUrl && filters.googleMapsUrl.length > 1028) {
        validateSearchResults.hasErrors = true
        validateSearchResults.errors?.push({
            field: 'googleMapsUrl',
    ...
    if (filters.googleMapsUrl && hasScriptTags(filters.googleMapsUrl)) {
        validateSearchResults.hasErrors = true
        validateSearchResults.errors?.push({
            field: 'googleMapsUrl',
    ...
    if (filters.healthcareProfessionalName === '' || (filters.healthcareProfessionalName && !filters.healthcareProfessionalName.trim())) {
        validateSearchResults.hasErrors = true
        validateSearchResults.errors?.push({
    ...
    if (filters.healthcareProfessionalName && filters.healthcareProfessionalName.length > 128) {
        validateSearchResults.hasErrors = true
        validateSearchResults.errors?.push({
            field: 'healthcareProfessionalName',
            errorCode: ErrorCode.INVALID_LENGTH_TOO_LONG,
    ...
    if (filters.healthcareProfessionalName && hasScriptTags(filters.healthcareProfessionalName)) {
        validateSearchResults.hasErrors = true
        validateSearchResults.errors?.push({
            field: 'healthcareProfessionalName',
            errorCode: ErrorCode.INVALID_INPUT,
    ...
    // if (filters.spokenLanguages && filters.spokenLanguages.length >= 2) {
    //     const languageCodeOccurances: any = {}
    //     for (let lang of filters.spokenLanguages) {
    //         !languageCodeOccurances[lang] ? languageCodeOccurances[lang] = 1 : 
    //         validateSearchResults.hasErrors = true
    //         validateSearchResults.errors?.push({
    //             field: 'spokenLanguages',
    //             errorCode: ErrorCode.INVALID_LANGUAGE_INPUT,
